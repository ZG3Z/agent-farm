...
class LangGraphTranslator:
    ...
    def _create_llm(self, api_key):
        if self.provider == "openai":
            return ChatOpenAI(model=self.model_name, api_key=api_key, temperature=self.temperature)
        elif self.provider == "anthropic":
            return ChatAnthropic(model=self.model_name, api_key=api_key, temperature=self.temperature)
        elif self.provider == "gemini":
            return ChatGoogleGenerativeAI(model=self.model_name, google_api_key=api_key, temperature=self.temperature)
    ...
    def translate(self, text, target_language):
        result = self.graph.invoke(TranslationState(
