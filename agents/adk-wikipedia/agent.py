...
class ADKWikipedia:
    ...
    def search(self, topic: str) -> dict:
        ...
        try:
            ...
            if wiki_content.startswith("No article found") or wiki_content.startswith(
                "Error"
            ):
                return {"topic": topic, "summary": wiki_content, "source": "Wikipedia"}
            ...
            return {"topic": topic, "summary": summary, "source": "Wikipedia via ADK"}
        except Exception as e:
            logger.error(f"Error: {e}", exc_info=True)
            return {"topic": topic, "summary": f"Error: {str(e)}", "source": None}
